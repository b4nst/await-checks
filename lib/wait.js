...
exports.getRunningChecksCount = (octo, namePattern, status) => __awaiter(this, void 0, void 0, function* () {
    const checks = yield octo.checks.listForRef(Object.assign({}, context.repo, { ref: context.ref }));
    console.log('Checks found:', _.map(checks, _.property('name')));
    if (!checks.data)
        return 0;
    return exports.countMatchingChecks(checks.data.check_runs, namePattern, status);
});
exports.countMatchingChecks = (checks, pattern, status) => _.filter(checks, c => {
    ...
    console.log(c.name, context.workflow, isNotSameJob, pattern, matchPattern, c.status, includeStatus);
